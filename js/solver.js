...
function nextState(state, next, n, d) {
	...
	nextState.approximation =
		nextState.length + approximate(nextState.path, n, d);
	return nextState;
}
...
function solvePlanar(solver) {
	...
	for (var i = 0; i < n; i++) {
		...
		for (var j = 0; j < n; j++) {
			...
			if (j === n - 1) {
				trongSo += d[i][j].toFixed(0);
			} else {
				trongSo += d[i][j].toFixed(0) + " - ";
			}
		}
		console.log("Trọng số là:", trongSo);
		trongSo = "";
	}
	solver(n, d);
}
		...
		if (current.id !== current.prevId) {
			setTimeout(
				function (state) {
					...
					var position = treeNetwork.getPositions([state.prevId])[
						state.prevId
					];
	...
	if (path.length > 3) {
		for (var iteration = 0; iteration < iterationsNumber; ) {
			...
			if (Math.random() < p) {
				...
				setTimeout(
					function (iteration, temperature, length) {
						info.innerHTML =
							"<b>Iteration: </b>" +
							iteration +
							"<br>" +
...
function displayState(state) {
	...
	info.innerHTML =
		"<b>Chiều dài:</b> " +
		state.length.toFixed(3) +
		"<br>" +
		"<b>Tính gần đúng:</b> " +
		state.approximation.toFixed(3) +
		"<br>" +
		"<b>Đường đi:</b> " +
		state.path.join(" > ");
}
